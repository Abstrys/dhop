...
```sh
DHOP='Dhop helps organize places'
```
...
1.  Get the sources. You can either use git::

        git clone https://github.com/Abstrys/dhop.git

    or download the .zip: https://github.com/Abstrys/dhop/archive/master.zip

1.  Run either ``install.sh`` or ``install.bat`` depending on your platform. Here's an example run
...
1.  If you used ``install.sh`` (Linux, Mac, BSD, etc.), you'll also need to add a line to your
...
1.  Once installed, test it out by typing ``dhop help``.
