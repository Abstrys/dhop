...
dhop is a command-line utility written in Ruby that provides a number of
...
All of these states are persistent and can be used even after your terminal
session has finished, your computer rebooted, etc.

Usage: dhop <cmd_or_location> [command_args]

Where <cmd_or_location> represents either a named location or one of the known
commands. Any further arguments on the command-line are considered parameters
...
set <name> [path]
  Sets a name for a given directory path. If no path is given, then the
...
push [path]
  ...
  the location referenced by path, if provided.
