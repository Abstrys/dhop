...
bool configure_set_cmd(CLI::App &app) {
   CLI::App* set_cmd = app.add_subcommand("set", "Set a name for a specified directory path.");
   return true;
}
...
bool configure_recall_cmd(CLI::App &app) {
   CLI::App* recall_cmd = app.add_subcommand("recall", "Return to the directory that was last marked with the 'mark' command.");
   return true;
}

bool configure_push_cmd(CLI::App &app) {
   CLI::App* push_cmd = app.add_subcommand("push", "Push the current working directory onto the directory stack, then go to the named location or path.");
   return true;
}

bool configure_pop_cmd(CLI::App &app) {
   CLI::App* pop_cmd = app.add_subcommand("pop", "Pops the last pushed location from the stack, and then transports you to that location.");
   return true;
}
